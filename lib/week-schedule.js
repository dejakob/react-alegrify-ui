...
class WeekSchedule extends Component {
    ...
    handleGenericMouseMove(eventData) {
        if (typeof this.state.selectedRangeIndex === 'number') {
            ...
            const moveDownCells = Math.round(yDiff / SELECTION_PIXEL_VALUES.cellHeight);
            const moveRightCells = Math.floor(xDiff / SELECTION_PIXEL_VALUES.cellWidth);
    ...
    handleTableClick(eventData) {
        ...
        if (eventData.target.localName === 'td') {
            ...
            const moveDownCells = Math.floor(yDiff / SELECTION_PIXEL_VALUES.cellHeight);
            const moveRightCells = Math.floor(xDiff / SELECTION_PIXEL_VALUES.cellWidth);
            const newWeekDayValue = moveRightCells;
...
function WeekScheduleViewRange(props) {
    ...
    const offsetLeft = weekDayIndexFrom * SELECTION_PIXEL_VALUES.cellWidth;
