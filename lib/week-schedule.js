...
class WeekSchedule extends Component {
    ...
    handleRangeMouseDown(eventData, rangeIndex) {
        ...
        this.setState({
            ...
            mouseDownOffset: { x: clientX, y: clientY }
        });
    }

    handleGenericMouseMove(eventData) {
        if (typeof this.state.selectedRangeIndex === 'number') {
            console.log('rangeIndex', this.state.selectedRangeIndex);

            //@Todo Caniuse clientX?
            const { clientX, clientY } = eventData;

            const xDiff = clientX - this.state.mouseDownOffset.x;
            const yDiff = clientY - this.state.mouseDownOffset.y;
            ...
            console.log('moveDownCells', moveDownCells);
            console.log('moveRightCells', moveRightCells);
        }
    }
...
function WeekScheduleViewRange(props) {
    ...
    return (
        <li
            ...
            style={{ top: `${offsetTop}px`, height: `${height}px`, left: `${offsetLeft}px` }}
